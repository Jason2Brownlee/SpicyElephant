...
var ScriptAttach = Class.create();
Object.extend(ScriptAttach, {
  at: function(url) {
...
function gsfn_search(form) {
  document.getElementById('gsfn_search_results').innerHTML = "Searching Get Satisfaction...";
  var url = "http://getsatisfaction.com/spicyelephant/searches"
  var params = []
...
function gsfnResultsCallback(json) {
  ...
  if (gsfn_topics.length == 0) {
    ...
    for (var t=0; t < gsfn_topics.length; t++) {
      topics_html.push(new Template(gsfn_topic_list.result).evaluate(gsfn_topics[t]));
    }
...
function gsfnTopicsCallback(json) {
  ...
  if (gsfn_topics.length == 0) {
    ...
    for (var t=0; t < gsfn_topics.length; t++) {
      topics_html.push(new Template(gsfn_topic_list.item).evaluate(gsfn_topics[t]));
    }
  }
  topics_html.push(gsfn_topic_list.close_tag);
